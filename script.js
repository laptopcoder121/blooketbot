...
document.getElementById("globalmsg").innerHTML = ("null"); // set to null for no msg
...
var cheats = {
    "Hack": [{
        ...
        action: function(a) {
            ...
            if (a.green) {
                setUserVal("cr", (function() {
                    var t = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
                    ...
                    t += "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
                    return t;
                })());
            } else {
                setUserVal("cr", 0);
            }
            a.innerText = a.green ? "Ungreen Host Screen" : "Turn Host Screen Green";
        }
    }, {
        type: "button",
        ...
        action: function(a) {
            ...
            a.innerText = a.frozen ? "Remove Freeze Password" : "Set Freeze Password";
        }
    }, {
        type: "input",
        name: "Set Crypto",
        ...
        computed: function(sel) {
            if (Object.keys(gameobject.c).length === Array.from(sel.children).length) {
                return false;
            }
        ...
        action: function(d) {
            setUserVal("tat", `${d}:${prompt("How much crypto do you want to steal?")}`);
        }
    }, {
        type: "input",
        name: "Advertise Text",
        ...
        action: function(d) {
            var adtext = prompt("Enter the text you would like to send (this will override your blook):");
            if (adtext) {
                if (adtext.includes(":")) {
                    alert("Ad text cannot include a colon!");
                    return;
                }
                setUserVal("b", "Parrot:" + (function() {
                    var r = "";
                    for (var i = 0; i < 500; i++) {
                        r += adtext + " ";
                    }
        ...
        action: function(stext) {
            setUserVal("tat", `${botinfo.name}:${Date.now()}${(new Array(1700).fill(stext+" ")).join("")}`);
        }
    }],
    "Gold": [{
            type: "button",
            ...
            action: function(d) {
                setUserVal("tat", `${d}:${prompt("How much gold do you want to steal?")}`);
            }
        },
        {
            type: "select",
            ...
            action: function(d) {
                setUserVal("tat", `${d}:swap:${prompt("What do you want to set it to?")}`);
            }
        }, {
            type: "input",
            name: "Set Gold",
    ...
    "Candy": [{
            ...
            action: function(d) {
                setUserVal("tat", `${d}:${prompt("How much candy do you want to steal?")}`);
            }
        },
        {
            type: "select",
    ...
    "Defense2": [{
        ...
        name: "Flood Alert Box",
        action: function(stext) {
            setUserVal("r", `${Date.now()}${(new Array(1700).fill(stext+" ")).join("")}`);
        }
    }],
    "Defense": [{
        type: "button",
    ...
    "Pirate": [{
        ...
        action: function(d) {
            setUserVal("tat", `${d}:${prompt("How many doubloons do you want to steal?")}`);
        }
    }, {
        type: "input",
        name: "Set Doubloons",
    ...
    "Cafe": [{
        ...
        action: function(d) {
            setUserVal("tat", `${d}:${prompt("What attack do you want(inspect, pay, etc)?")}`);
        }
    }, {
        type: "input",
        name: "Flood Alert Box",
        action: function(stext) {
            setUserVal("up", `a:${Date.now()}${(new Array(1700).fill(stext+" ")).join("")}`);
        }
    }],
    "Brawl": [{
        type: "button",
        ...
        name: "Flood Alert Box",
        action: function(stext) {
            setUserVal("up", `__proto__:${Date.now()}${(new Array(1700).fill(stext+" ")).join("")}`);
        }
    }],
    "Racing": [{
        type: "button",
        ...
        action: function(a) {
            ...
            a.innerText = a.frozen ? "Unfreeze Scoreboard and Attacks" : "Freeze Scoreboard and Attacks";
        }
    }, {
        type: "input",
        ...
        action: function(d) {
            setUserVal("tat", `${d}:${prompt("Which attack do you want to perform(rocket,etc)?")}`);
        }
    }],
    "Classic": [{
        type: "button",
    ...
    "Toy": [{
            ...
            action: function(d) {
                setUserVal("tat", `${d}:${prompt("How many toys do you want to steal?")}`);
            }
        },
        {
            type: "select",
...
var global = [{
    ...
    action: function(a) {
        ...
        setUserVal(fvals[botinfo.type], makeLongText(1.5e+5));
...
document.body.addEventListener("mousemove", function(e) {
    if (dragging) {
        ...
        document.querySelector(".chat").style.left = parseInt(document.querySelector(".chat").style.left) + e.clientX - prevpos.x + "px";
        document.querySelector(".chat").style.top = parseInt(document.querySelector(".chat").style.top) + e.clientY - prevpos.y + "px";
...
async function useToken(token) {
    ...
    await connect(gid, name, document.getElementById("icogmode").getAttribute("checked"), {
        success: !0,
        fbShardURL,
        fbToken
    });
...
function addChatMessage(a) {
    ...
    document.querySelector(".chatcontainer").insertBefore(d, document.querySelector(".chatcontainer > div"));
}
//CODE END
//CHECKMARK CODE
...
async function connect(gid, name, icog, reqbody = !1) {
    ...
    const body = reqbody ? reqbody : await fetch("join", {
        body: JSON.stringify({
            id: gid,
            name: name
        }),
